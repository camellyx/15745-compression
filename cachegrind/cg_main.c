...
void log_1IrNoX_1Dr_cache_access(InstrInfo* n, Addr data_addr, Word data_size)
{
   ...
   n->parent->Dr.a++;
   //print_value(data_addr, data_size);   
}
...
static void cacheCompression (cache_t2 D)
{
   //gpekhime
   static HChar buf1[128];
   ...
   for(ii = 0; ii < D.sets; ii++){
       UWord * set4Sizes = &(D.compSizes[ii * D.assoc * 2]);
       UWord * set = &(D.tags[ii * D.assoc * 2]);
       VG_(printf)("set %02x: ", ii);
       //Int waysUsed = 0;
       for(jj = 0; jj < D.assoc * 2; jj++){
         UChar valid = set[jj] ? 1 : 0;
         VG_(printf)("way %02x v: %01d compSize: %02d ", jj, valid, set4Sizes[jj]);
         if(valid){
            totalSize += D.line_size;
            compSize += set4Sizes[jj];
            waysUsed++;
	 } 
       }
       if(waysUsed > D.assoc)
           compRatio +=  (double)waysUsed / (double)D.assoc;
       else
           compRatio += 1.0;
   ...
   l1 = ULong_width(totalSize);
   VG_(percentify)(totalSize, compSize, 1, l1+1, buf1);
   VG_(umsg)("Compression ratio: %s\n", buf1);
   VG_(percentify)(waysUsed, D.sets * D.assoc, 1, l1+1, buf1);
   VG_(umsg)("ECR: %s\n", buf1);
